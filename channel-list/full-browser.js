(() => {
 ...
 const titles={live:'Kanali',movies:'Filmovi',series:'Serije / epizode'};
 ...
 const label=c=>(labels[c.code]||c.code)+' · '+c.code;
 ...
 function renderCountries(){
  ...
  }if(!filtered.length)$('categoryList').textContent='Nema rezultata za ovu državu ili kategoriju.';
 }
 function renderCategories(){
  $('groupSelect').replaceChildren();const all=document.createElement('option');all.value='';all.textContent='Sve kategorije';$('groupSelect').append(all);
 ...
 function pager(total){
  ...
  button('Prva',1,page<=1);button('Prethodna',page-1,page<=1);
  const info=document.createElement('span');info.textContent=page+' / '+pages;$('channelPages').append(info);
  button('Sljedeća',page+1,page>=pages);button('Zadnja',pages,page>=pages);
 }
 function renderRows(rows){$('channelsList').replaceChildren();if(!rows.length)return setMessage('Nema rezultata.');for(const item of rows){const row=document.createElement('div');row.className='channel-row';const n=document.createElement('b');n.textContent=item.name;const cat=document.createElement('small');cat.textContent=item.category;row.append(n,cat);$('channelsList').append(row);}}
 async function renderPage(){
  const token=++revision; if(!country){$('selectedCategory').hidden=true;$('channelPages').replaceChildren();return setMessage('Izaberite državu ili regiju lijevo.');}
  const total=results===null?count():results.length,pages=Math.max(1,Math.ceil(total/PAGE_SIZE));page=Math.min(page,pages);
  $('selectedCategory').hidden=false;$('selectedCategory').textContent=label(country)+' · '+titles[kind]+' · '+total.toLocaleString()+' stavki';pager(total);setMessage('Učitavanje…');$('channelsList').setAttribute('aria-busy','true');
  try{
   ...
   if(token!==revision)return;renderRows(rows);status('Kompletan katalog · '+manifest.total.toLocaleString()+' stavki');
  }catch(e){if(token!==revision)return;setMessage('Učitavanje nije uspjelo. Pokušajte ponovo.');const b=document.createElement('button');b.textContent='Pokušaj ponovo';b.onclick=renderPage;$('channelsList').append(b);}
 ...
 async function refresh(){
  ...
  const token=++revision,list=parts(),found=[];$('channelPages').replaceChildren();setMessage('Pretraživanje svih stavki izabrane države…');$('channelsList').setAttribute('aria-busy','true');
  try{for(let i=0;i<list.length;i++){if(token!==revision)return;const p=list[i],names=await getNames(p);if(token!==revision)return;for(const name of names)if(norm(name).includes(q))found.push({name,category:p.category});status('Pretraživanje: '+Math.round((i+1)/list.length*100)+'%');}if(token!==revision)return;results=found;renderPage();}
  catch(e){if(token!==revision)return;setMessage('Pretraga nije završena. Pokušajte ponovo.');const b=document.createElement('button');b.textContent='Ponovi pretragu';b.onclick=refresh;$('channelsList').append(b);status('');}
  finally{if(token===revision)$('channelsList').setAttribute('aria-busy','false');}
 }
 ...
 async function init(){
  try{const r=await fetch(base+'manifest.json');if(!r.ok)throw Error('Manifest unavailable');manifest=await r.json();if(!manifest.complete)throw Error('Incomplete catalog');
   ...
   status('Kompletan katalog · '+manifest.total.toLocaleString()+' stavki');changeKind('live');
  }catch(e){$('categoryList').textContent='Katalog nije učitan.';setMessage('Osvježite stranicu ili pokušajte ponovo.');const b=document.createElement('button');b.textContent='Pokušaj ponovo';b.onclick=init;$('categoryList').append(b);}
 }
 init();
})();
